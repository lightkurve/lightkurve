...
def test_CotrendingBasisVectors_nonretrieval():
    ...
    cbvs = CotrendingBasisVectors(dataTbl, cbvTime)
    cbvs, trimmed_lc = cbvs.align(sample_lc, trim_lc=True)
    assert np.all(trimmed_lc.cadenceno == cbvs.cadenceno)
    assert len(cbvs.cadenceno) == 4
    assert len(trimmed_lc.flux) == 4
    cbv_designmatrix = cbvs.to_designmatrix(cbv_indices=[1])
    assert np.all(cbv_designmatrix['VECTOR_1'] == [1.0, 2.0, 3.0, 6.0])
...
def test_cbv_retrieval():
    ...
    cbvs = get_tess_cbvs(sector=10, camera=2, CCD=4, cbv_type = 'SingleScale')
    ...
    cbvs = get_tess_cbvs(sector=10, camera=2, CCD=4, cbv_type = 'MultiScale', band=2)
    assert isinstance(cbvs, TessCotrendingBasisVectors)
    ax = cbvs.plot_cbvs('ALL')
    assert isinstance(ax, matplotlib.axes.Axes)
    assert cbvs.band == 2
    
    cbvs = get_tess_cbvs(sector=8, camera=3, CCD=1, cbv_type = 'Spike')
    assert isinstance(cbvs, TessCotrendingBasisVectors)
    ax = cbvs.plot_cbvs('ALL')
    assert isinstance(ax, matplotlib.axes.Axes)
    ...
    with pytest.raises(AssertionError):
        cbvs = get_tess_cbvs(sector=10, camera=2, CCD=4, cbv_type = 'MultiScale')
    # Band specified for SingleScale, this should also error
    with pytest.raises(AssertionError):
        cbvs = get_tess_cbvs(sector=10, camera=2, CCD=4, cbv_type = 'SingleScale', band=2)
    # Improper CBV type request
    with pytest.raises(Exception):
        cbvs = get_tess_cbvs(sector=10, camera=2, CCD=4, cbv_type = 'SuperSingleScale')

    cbvs = get_kepler_cbvs(mission='Kepler', quarter=8, module=16, output=4)
    ...
    cbvs = get_kepler_cbvs(mission='K2', campaign=15, channel=24)
    assert isinstance(cbvs, KeplerCotrendingBasisVectors)
    ax = cbvs.plot_cbvs('ALL')
    assert isinstance(ax, matplotlib.axes.Axes)
