...
class TargetPixelFile(object):
	...
	def estimate_target_aperture(
		...
		if (center_col is None) | (center_row is None):
			...
			SkyCoord(self.skycatalog['RA'][0], self.skycatalog['DEC'][0], unit="deg")
			)
	...
	def estimate_contamination(
		...
		PRF_base = self.prf
		prf_model = PRF_base.prf_model(center_row = self.skycatalog['Row'].value, center_col = self.skycatalog['Column'].value, 
				scale=scale, rotation_angle=rotation_angle)
		return PRF_base.get_contamination(prf_model, aperture, self.skycatalog['Relative_Flux'].value)
