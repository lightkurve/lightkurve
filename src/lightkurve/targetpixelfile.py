...
class KeplerTargetPixelFile(TargetPixelFile):
	...
	**kwargs : dict
	...
	def prf(self):
		...
		return KeplerPRF(self.column, self.row, self.shape[1:3], self.channel)
	...
	def estimate_completeness(self,
		...
		# ValueError instead probably
		if aperture is None:
			print('Need to specify an aperture')
			return None
		...
		#self, prf_model: npt.ArrayLike, aperture: npt.ArrayLike, idx: int = 0
	#)
...
class TessTargetPixelFile(TargetPixelFile):
	...
	def prf(self):
		...
		return TessPRF(self.column, self.row, self.shape[1:3], self.camera, self.ccd)

	# @property
	'''
	I think this is valid for both Tess and Kepler if I make general names.
	I can move this to the generic tpf class
	Check if K2 has the same header keywords 1/2CRV4P and 1/2CRPX5'''
	def simple_aperture(self,
		center_col:  Union[float, list[float], npt.ArrayLike, SkyCoord, list[SkyCoord], None] = None, 
		center_row:  Union[float, list[float], npt.ArrayLike, SkyCoord, list[SkyCoord], None] = None,
		scale: float = 1.0,
		rotation_angle: float = 0.0,
		min_completeness: float = 0.9, 
		plot: bool = False)-> npt.ArrayLike:
		...
		if (center_col is None) | (center_row is None):
			center_col, center_row = self.wcs.world_to_pixel(SkyCoord(self.ra, self.dec, unit='deg'))
		...
		if isinstance(center_col, SkyCoord) | isinstance(center_row, SkyCoord): 
			center_col, center_row = self.wcs.world_to_pixel(SkyCoord(center_col.deg, center_row.deg, unit='deg'))
			center_col = float(center_col) + self.column
			center_row = float(center_row) + self.row
		
		TessPRF = self.prf
		prf_model = TessPRF.prf_model(center_col = center_col, center_row = center_row, 
			scale=scale, rotation_angle=rotation_angle)

		aperture = TessPRF.get_simple_aperture(prf_model, min_completeness=min_completeness, plot=plot)
